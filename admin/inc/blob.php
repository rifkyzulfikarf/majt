...
class koneksi {
	...
	function __construct(){ 
		$this->konek=new mysqli("localhost", "root", "", "majt");
...
function cek_login($user,$pass){
	...
	$user = e_code($user);
    $pass = e_code($pass);
	if( $hmm = $koneksi->runQuery("SELECT `pemakai`.`id`,`pemakai`.`user`,`pemakai`.`kunci`,`pemakai`.`status`, `karyawan`.`nama` , `karyawan`.`id` as `id_kar`, `karyawan`.`level`, `karyawan`.`area`  FROM `pemakai` INNER JOIN `karyawan` ON `karyawan`.`id` = `pemakai`.`id_karyawan` WHERE `pemakai`.`user`='$user' && `pemakai`.`kunci`='$pass' ") ){
		if( $hmm->num_rows > 0 ){
			$rs=$hmm->fetch_array();
